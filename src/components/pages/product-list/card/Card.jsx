    ...
    return (
      <section className="cards">
        <section className="card-zoomin-hover flex gap-[20px] border border-[#e0e0e0] rounded-md cursor-pointer hover:shadow-lg">
          ...
          <Link
            to={`/${item.id}`}
            className="flex-[4] md:flex-[2] min-h-[250px] overflow-hidden"
          >
            <img
              src={item.img}
              alt={item.title}
              className="block italic w-full min-h-full  rounded-tl-md rounded-bl-md object-cover"
            />
          </Link>

          {/* text container */}
          <div className="relative flex-[5] md:flex-[3] flex flex-col md:flex-row min-h-[300px] md:min-h-[250px]">
            <div className="flex-[4] pt-4 pr-2">
              {item.isSponsored && (
            ...
            <div className="md:border-l md:border-[#e0e0e0] flex-[2] p-2 flex flex-col">
              ...
              <div className="absolute max-md:left-0 bottom-20 text-gray-600 font-medium font-montserrat">
                <b className=" text-gray-900 text-[1.1rem]">$ {item.price}</b> /
                month
              </div>
            </div>
          </div>
